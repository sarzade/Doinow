...
نسخه‌ها خودکار شماره می‌خورند (`v1.0`، `v1.1`، …). برای انتشار نسخه جدید: تب **Actions** ← ورک‌فلو **Release** ← دکمه **Run workflow** (می‌توانی یادداشت هم اضافه کنی). پوش معمولی به `main` فقط بیلد CI می‌سازد و Release نمی‌سازد.
